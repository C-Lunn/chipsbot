...
Object.assign(String.prototype, { //adds functionality to search for multiple words in one function - i.e. contains any of the words given
    contains() {
        var rv = false;
...
rtm.on('message', (event) => { //this is a callback that occurs on every message sent to every channel the bot is in
    ...
    if (event.type == 'message') {
        ...
        } //if untagged but someone says chips or linux....
        ...
        if (MsgArgs[0].includes("<".concat(BotID.toLowerCase(), ">"))) {  //if first arg of message is @chipsbot then start doing stuff
            if(event.text.includes("the usual")) {
                event.text = "<".concat(BotID.toLowerCase(), ">") + "lunchtime set 12:30";
...
function getUserIntention(event) {
    ...
    } else if (msgString.contains("lunchtime", "subscribe", "lunch", "souscrire", "abonnieren", "подписываться", "订阅", "tanysgrifiwch")) {
...
function betIntentHandler(event) {
    rtm.sendMessage("When the fun stops, STOP.", event.channel);
}

var z = 30;
async function words(MsgArgs, event) {

    res = await web.chat.postMessage({
        "text": "`C` `H` `I` `P` `S` (1 vowel, 4 consonants)  you have 30 seconds.....",
        "channel": event.channel,
        "as_user": true,
    });

    cdi = setTimeout(function () {
        updateCountdown(res.ts, event.channel, 30);
    }, 1000);

}

async function updateCountdown(msgId, channel, z) {
    ...
    web.chat.update({
        "ts": msgId,
        "text": "`C` `H` `I` `P` `S` (1 vowel, 4 consonants)  you have " + z + " seconds.....",
        "channel": channel,
        "as_user": true,
    });
    ...
    if (z != 0) {
        setTimeout(function () {
            updateCountdown(msgId, channel, z);
        }, 1000);
    }
}
...
function lunchtimeIntentHandler(event) {
    ...
    if (stringtoParse.includes("set")) {
        ...
        setLunchtime(MsgArgs, event, stringtoParse, true);
    } else if (stringtoParse.contains("subscribe", "souscrire", "abonnieren", "подписываться", "订阅", "tanysgrifiwch")) {
...
function helpIntentHandler(MsgArgs, event) { //comment
    if (MsgArgs.length == 2) {
        rtm.sendMessage(
            `CHIPSBOT COMMANDS: \n
